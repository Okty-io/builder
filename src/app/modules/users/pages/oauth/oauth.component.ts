...
export class OauthComponent implements OnInit {
    ...
    private getParams(): { code: string, state: string } {
        ...
        if (sessionStorage.getItem('api_state') !== state) {
            throw new Error('Auth failed');
        }
        sessionStorage.removeItem('api_state');
    ...
    private loginWithGithub() {
        ...
        this.api.post('login', {code, state, provider: 'github'})
            .subscribe(
    ...
    private loginWithGitlab() {
        ...
        this.api.post('login', {code, state, provider: 'gitlab'})
            .subscribe(
                (response: { token: string }) => this.handleSuccess(response),
                (error: HttpErrorResponse) => this.handleError(error)
            );
